...
The card proposes three things picked from the engine's four registers, and a button press
(해 · 미뤄 · 빼) is the verdict. Everything that decides what a card means lives here:
the register shapes, the prompt, the proposal parser that refuses anything not grounded
in a register's sources, the Block Kit layout, and the button-payload parser that returns
a Rejected value instead of raising. The graph (card.py) and the socket (card.py main)
are thin because this file is strict.
"""
...
class Proposal(BaseModel):
    """One thing the card proposes. `source_note` is not a hint — it must be one of the
    strings its register listed, which is what parse_proposals enforces. The attribute is
    `register_` only because a field literally named `register` would shadow BaseModel.register
    (pydantic 2.13 then treats the inherited method as the field's default); the JSON name
    stays `register` via the alias."""
...
def build_prompt(registers: Registers) -> str:
    """The whole proposal task: the registers with their allow-lists, and the rules. The model
    only picks and justifies — invention is refused later, in parse_proposals."""
    sections = []
    for name in REGISTER_NAMES:
        text = registers.texts.get(name, "")[:PROMPT_SECTION_BUDGET]
        listed = "\n".join(f"{i}. {s}" for i, s in enumerate(registers.sources.get(name, []), 1))
        sections.append(f"[{name}]\n{text}\n\nsources:\n{listed}")
    rules = (
        "아래 네 개 레지스터는 엔진이 오늘 본 것들이다. 이 안에서 오늘 제안 셋(정확히 3개)을 골라라.\n"
        "\n"
        "출력은 JSON 하나뿐이다:\n"
        '{"proposals": [{"title": "짧은 명사구", "why": "한 문장", "source_note": "…", '
        '"register": "…"}, …]}\n'
        "\n"
        "규칙:\n"
        "- 새로운 사실을 지어내지 마라. 레지스터에 있는 것만 제안할 수 있다.\n"
        "- source_note 는 그 섹션의 sources 목록 원소 하나를 글자 그대로 복사한 것이다. "
        "목록에 없는 문자열은 무효다.\n"
        '- 좋은 예: 목록이 `1. next_action` 이면 "source_note": "next_action" 이다.\n'
        '- 나쁜 예: "source_note": "next_action — task_list: 정본…" — 본문 줄이지 목록의 '
        "원소가 아니므로 무효다.\n"
...
def parse_proposals(llm_json: str, registers: Registers) -> list[Proposal]:
    ...
    for proposal in proposals:
        allowed = registers.sources.get(proposal.register_) or []
        if proposal.source_note not in allowed:
            raise ValueError(
                f"proposals: source_note {proposal.source_note!r} not in register "
                f"{proposal.register_!r} sources"
            )
    return proposals
...
def build_blocks(proposals: list[Proposal], verdicts: Iterable[ButtonVerdict] = ()) -> list[dict]:
    """Block Kit for the card: one section and one button row per proposal. A judged row shows
    its mark instead of its buttons — the card is edited in place as 판정들 arrive."""
    by_idx = {v.idx: v for v in verdicts}
    blocks: list[dict] = [
        {"type": "header", "text": {"type": "plain_text", "text": CARD_TITLE, "emoji": True}}
    ]
        ...
        blocks.append(
            {
                ...
                "text": {
                    ...
                    "text": (
                        ...
                        f"_{proposal.register_} · {proposal.source_note}_"
                    ),
                },
            }
        )

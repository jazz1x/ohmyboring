...
async fn check_duplicate(
    ...
    if let Some(found) = newest_same_session(same_session).or_else(|| others.into_iter().next()) {
...
/// Once a session's notes supersede each other it has several files; the one to compare
/// against is the latest allocated, not whichever `read_dir` lists first.
fn newest_same_session(matches: Vec<DuplicateMatch>) -> Option<DuplicateMatch> {
    matches
        .into_iter()
...
mod tests {
    ...
    use super::{
        ...
        needs_dedup, newest_same_session, parse_remember_note, parse_supersedes,
    ...
    fn session_match(path: &str) -> DuplicateMatch {
        DuplicateMatch {
    ...
    #[test]
    fn newest_same_session_picks_the_latest_wiki_id_in_any_order() {
        let a = "/v/wiki/wiki-0001.md";
        let b = "/v/wiki/wiki-0002.md";
        for order in [[a, b], [b, a]] {
            let found = newest_same_session(order.map(session_match).into()).unwrap();
            assert_eq!(found.source_path, b, "{order:?}");
        }
    }

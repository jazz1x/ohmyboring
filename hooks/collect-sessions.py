...
- 마커: distill-session.py 와 같은 ~/.cache/olympus-distill/<sid>.ts. 있으면 skip(이미 함).
...
MARK_DIR = os.path.expanduser("~/.cache/olympus-distill")
...
def main():
    ...
    try:
        req = urllib.request.Request(f"{HERMES_RS}/sync", data=b"", method="POST")

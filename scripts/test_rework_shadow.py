...
BODY = (
    "the connection pool died because deadpool recycled a socket the server had already "
    "closed and the retry loop kept handing the same broken object back to every caller, "
    "so the fix was to drop the idle timeout below the load balancer's own "
) * 3


def build(tmp):
    v = pathlib.Path(tmp) / "wiki"
    v.mkdir()

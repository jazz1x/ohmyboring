...
```text
            書き込み (ゲート)                             読み出し (オープン·高速)
  ┌────────────────────────────────┐          ┌──────────────────────────────┐
  セッション ──蒸留──▶ vault/raw   │          │  「これ何だっけ?」            │
  (Claude Code,   ──compile──▶ vault/wiki ◀────────  make ask · recall.py      │
   SessionEnd フック)(キュレーション) *.md    │         · Slack · MCP recall   │
  └────────────────────────────────┘  (1級)   └──────────────────────────────┘
                                         │
                          (任意) ───────┴──── DRUDGE_VECTOR=on
                                   pgvector: 埋め込み + グラフ RAG
```
...
① 終了/中断  →  distill-session.py（SessionEnd/Stop フック）
                セッション蒸留 → vault/raw （エンジン；または opt-in でエージェント）
② compile    →  raw → vault/wiki （LLM キュレーション: タイトル・タグ・repo/<slug>）
                [スケジューラ · make sync · セッション直後]
③ recall     →  make ask / recall.py / Slack / MCP  →  vault/wiki を直接読む
                （+ DRUDGE_VECTOR=on なら pgvector 類似度 & グラフ）
```

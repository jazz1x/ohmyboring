...
- **ローカル専用** — 埋め込みも合成もローカルの LLM サーバー（既定 Ollama）。外部API・トークンは0。ノートはディスクの外に出ない。
...
|---|---|---|---|---|:---:|
| 1 | **LLM サーバー**（ホスト・OpenAI互換） | 埋め込み `bge-m3`（1024次元）· 合成 `gemma4:12b` — 既定 Ollama、LM Studio/vLLM 等に交換可 | ホストプロセス | `127.0.0.1:11434/v1` | 必須[^ollama] |
...
[^ollama]: OpenAI互換 `/v1` サーバーがホストで起動している必要がある（既定 `ollama serve`）。コンテナは `host.docker.internal` 経由で到達。別ランタイムは `DRUDGE_LLM_BASE_URL` で指定（例 LM Studio `:1234/v1`）。
...
| **LLM ランタイム**（OpenAI互換） | ローカルの埋め込み・合成 | 既定 **Ollama**（`ollama --version` · [ollama.com](https://ollama.com)）。LM Studio/vLLM 等も可 — `DRUDGE_LLM_BASE_URL` で指定 |
...
| 方式 | やり方 | いつ |
|---|---|---|
| **Docker**（既定） | `make up` — postgres + drudge（+agent）コンテナ | 最も簡単。推奨 |
| **ネイティブ** | ホストに Postgres（pgvector）直接 + `cd drudge && cargo run --release -- serve` | コンテナ不要、またはデバッグ/開発時 |

ネイティブは env を合わせるだけ — `PG_DSN`（ホスト pg）· `DRUDGE_LLM_BASE_URL`（LLM サーバー）· `DRUDGE_VAULT_DIR` · `DRUDGE_SOURCE_DIRS`。drudge は単一の静的バイナリでどこでも動く。CLI 直接: `cargo run -- ask "..."` · `cargo run -- sync` など。
...
## Nous Hermes Agent 連携（任意）

drudge は外部 [Nous Hermes Agent](https://github.com/NousResearch) の **MCP メモリバックエンド**になれる。エージェントが脳（何を・いつ取り込む/回収するかを判断し、自前でスキルを生成）、drudge はその機械作業（compile・埋め込み・グラフ）をシステム化した手。

1. エージェント起動: `docker compose --profile agent up -d`（外部 `hermes-agent` イメージが必要 — 本リポジトリ非同梱、別途ビルド）。
2. エージェント設定（`~/.hermes/config.yaml`）に drudge を MCP サーバーとして登録:
   ```yaml
   mcp_servers:
     drudge:
       url: http://drudge:7700/mcp   # 同一 compose ネットワーク: サービス名で到達
       transport: http               # ホストから直接なら http://localhost:7700/mcp
   ```
3. エージェントが使う 3 つの MCP ツール（drudge `/mcp`）:

| ツール | 方向 | 動作 |
|---|---|---|
| `recall{query}` | 読み | ベクトル+グラフで過去の経験を回収 |
| `remember{text,title?}` | 書き | vault/raw にノートを取り込み（次の sync で吸収） |
| `sync{}` | 駆動 | compile → ingest → extract を 1 回 |

> **取り込みトリガーは 3 層で共存**: ① drudge の 4h スケジューラ（土台、`DRUDGE_SYNC_HOURS`）· ② SessionEnd フック（セッション捕捉）· ③ エージェント MCP（能動）。エージェントが `remember`/`sync` で能動的に取り込み、スケジューラとフックが安全網。エージェントなしの ①② だけでもコアは完結。
...
| `DRUDGE_LLM_BASE_URL` | `http://localhost:11434/v1` | OpenAI互換 LLM サーバー（Ollama 既定 · LM Studio `:1234/v1` 等） |
| `DRUDGE_LLM_API_KEY` | — | 認証が必要な provider のみ（Ollama/LM Studio は不要） |
| `DRUDGE_LLM_MODEL` | `gemma4:12b` | 合成モデル（任意） |

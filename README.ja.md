...
デフォルトの wiki-first モード（`BORING_VECTOR=off`）では、4 つの tool が pgvector バックエンドを必要とし、`BORING_VECTOR=on` を設定するまで JSON-RPC `-32603` を返します: `neighbors`、`claims`、`corpus_status`、`brief`。残りの 7 つ（`recall`、`ask`、`remember`、`forget`、`sync`、`config_get`、`classify_repo`）は `vault/wiki` を直接使用します。
...
- `ask` / `brief` — LLM を実行する唯一の tool: `ask` は出典を引用して質問に答え（wiki-first モードで動作）、`brief` *(`BORING_VECTOR=on` が必要)* は新しい順を優先した作業ブリーフィングです。

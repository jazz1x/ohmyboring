...
- **ゲート**（ローカル `make guard` = CI の `rust-gate`）: `rustfmt --check` + `clippy -D warnings`（`unsafe` forbid + pedantic）+ `cargo test`（スタック非依存）。サプライチェーン: `make deny`。（`make guard` 通過 ≠ CI 通過 — CI は gitleaks + cargo-deny + trivy も実行。）
- **pre-commit**: 一度 `pre-commit install`（ファイル衛生 + gitleaks + fmt/clippy/test + py-compile）。
- **CI**: PR・main push のたびに `rust-gate` + `gitleaks` + `cargo-deny` + `trivy`、4つすべて必須（admin 回避不可）。

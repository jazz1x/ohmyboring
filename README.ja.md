...
| `BORING_EVENT_LOG` | ローカル NDJSON ワークフローイベントのスプール。デフォルトは `~/.cache/oh-my-boring/events.ndjson` |
| `BORING_EVENT_DB_MIRROR` | DB ミラーリングモード。デフォルトは有効、NDJSON のみにする場合は `0`/`false`/`off`/`no` |
| `BORING_EVENT_SINK_URL` | 任意の DB ミラーリング先。デフォルトは `$BORING_URL/events` |
...
構造化イベントは distill、collector/worker、`doctor`/`readiness`、`guard`、`eval` から記録されます。memory-ingest イベントには Rust ワークフローグラフ契約に沿った `workflow=memory_ingest`、`workflow_node`、`workflow_outcome` フィールドが付きます。イベントはまずローカル NDJSON スプールに append され、`BORING_EVENT_DB_MIRROR=0` で無効化しない限り OpenTelemetry 形式のログレコードとしてローカル DB にミラーされます。DB view は HTTP `/events` または MCP `events`、エンジン停止時も含むファイルスプールは `make events` で確認します。
...
- `events` *(`BORING_VECTOR=on` が必要)* — DB に OpenTelemetry 形式でミラーされた最近の workflow/adapter イベントを返します。component、event、status、run_id、workflow、since_hours でフィルタできます。

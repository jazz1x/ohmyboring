...
**ohmyboring은 내가 어떻게 풀었는지를 기억합니다.** Claude Code / Kimi Code 세션과 적재 가능한 Codex 트랜스크립트를 로컬의 사람이 읽는 위키로 바꾸고, *"전에 이거 어떻게 했더라"* 싶을 때 다시 꺼내줍니다 — 어떻게 풀었는지를 담아 두는 것이고, 회수 정밀도는 공개적으로 재는 중입니다(`make peek`). **클라우드 0 · 로컬 LLM 친화.**
...
| `BORING_DOOR_URL` | 아침 카드와 Claude Code SessionStart 훅이 문을 찾는 URL — 두 소비자 모두 필수다. 없으면 카드가 기동 거부(exit 2)하고 훅은 `오늘 승인한 것` 절을 뺀다. 소비자는 기본값을 두지 않는다. `schedule-card.sh` 와 `doctor.sh` 의 `/health` 확인만 따로 `http://127.0.0.1:7710` 으로 떨어진다 |

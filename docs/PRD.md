...
**And then the channel changed anyway.** On the window's second day the owner instructed that nothing wait on a date, and the injection channel was changed under that instruction: #365 taught `/search` to hand over the claims behind a hit (opt-in, default off — by itself not a channel change), and #368 made the hook ask for one per hit and render it as its own line under the snippet. Injections from that point carry roughly 60–120 characters per hit that earlier ones did not, and what they carry is a different *kind* of material — the settled thing rather than the prose around it. This is not the scorer; it is the thing being scored.

What that costs is specific, and it is not the sample. Rows already recorded stay valid as a measurement of the old form, and rows recorded from here stay valid as a measurement of the new one. What is gone is the single two-week verdict §2 pre-registered, because "the injection channel in its current form" no longer names one form. Two readings are available and the contract does not pick between them: **split** the window at the change and report both halves against the floors, accepting that neither half will reach 20 sessions, or **restart** the window from the change and accept a close past `verdict_core.WINDOW_UNTIL`. The owner picks; this paragraph exists so the pick is made knowingly rather than discovered at the close.

Recorded here because the clause above demands it of the scorer, and a change to the channel is the larger of the two. The instruction that authorised it is the owner's and is not in dispute — what is recorded is the consequence, which no instruction removes.
...
| 2026-09-17 | D11 — third window (09-12→09-26) reset: the SessionEnd hook was registered as `nohup … &`, and a process backgrounded in a non-interactive shell reads `/dev/null`, so the payload never reached the script; `>/dev/null` swallowed the error. What made the earlier windows look alive was the backfill collector forging `hook_event_name: SessionEnd`, removed in `#312` — so D9 measured the collector's schedule, not session ends. Registration repaired 2026-09-17T01:51:35Z; `LEDGER_REPAIR_AT` moves to that instant, not a commit, because no repository holds `~/.claude/settings.json`. New window registered at sample 0/0. Double-firing ruled out before the reset: of 858 sessions in `distill_resolution`, 27 have repeat rows and the smallest gap is 353s — the collector's period, not a second delivery | `#337` `#338`, this file's history |

...
#
# Brand = ohmyboring; install dir = ~/oh-my-boring (the ohmyzsh ~/.oh-my-zsh homage; override with BORING_HOME).
# Idempotent: re-running updates the clone and skips already-wired hooks. Set BORING_WIRE=0 to skip the
...
if [ -d "$BORING_HOME/.git" ]; then
  say "Updating existing install at $BORING_HOME"
  git -C "$BORING_HOME" pull --ff-only 2>/dev/null || warn "pull skipped — could not fast-forward (network/auth/diverged branch or local changes). Keeping your working tree."
else
  say "Cloning ohmyboring → $BORING_HOME"
  git clone "$REPO" "$BORING_HOME"
fi
cd "$BORING_HOME"
...
if [ "${BORING_WIRE:-1}" = 1 ]; then
  ...
  warn "BORING_WIRE=0 — skipped agent wiring. See README 'Agent adapters' to add hooks + .mcp.json yourself."
fi

...
| `BORING_DOOR_URL` | where the morning card and the Claude Code SessionStart hook find the door — required by both consumers. Without it the card refuses to start (exit 2) and the hook skips the `오늘 승인한 것` section. Only `schedule-card.sh` assumes `http://127.0.0.1:7710`, for its `/health` check |

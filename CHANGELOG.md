...
### Added
- **소유자 토큰이 owner 쓰기를 막는 문지기가 된다** — 목적은 owner 가 쓴 노트가 owner 만 대체·수정하게 하는 것이고, owner 가 아닌 호출이 owner 를 작가·판정으로 주장하지 못하게 하는 것이다(`drudge/src/serve/owner.rs` — `may_rewrite`·`refused_supersedes`). `X-Boring-Owner-Token` 헤더가 엔진의 `BORING_OWNER_TOKEN` 과 일치하지 않으면 `owner (as author or judge) needs the owner door token in x-boring-owner-token` 으로 거절되는데, 이 거절은 이벤트를 남기지 않는다 — 이벤트로 남는 것은 `owner_supersede_refused`·`forget_refused` 뿐이다.

...
### Fixed
- **doctor 가 아침 카드가 오늘 나갔는지 말한다** — 09-24~26 세 아침이 연속 실패했는데 doctor 에 카드 항목이 하나도 없어 발견이 늦었다. `scripts/schedule-card.sh` 의 시작·종료 마커에 ISO 시각을 붙이고, card.py 가 게시 직후 `[card] posted ts=` 한 줄을 남긴다(finished 는 CARD_WAIT_HOURS 23시간 대기 뒤에야 찍혀, 성공한 실행이 대부분의 날 동안 finished 없이 남는다). doctor 는 `${BORING_CARD_LOG}` 의 마지막 시작 시각과 뒤따르는 finished·posted 줄로 성공·실패·미실행을 가리고, 마지막 시작이 지난 08:05(08:00 실행 + 발송 여유)보다 오래됐으면 「안 나감」으로 본다. 시험 8개 + 변이 2개.

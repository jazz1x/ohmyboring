...
### Added
- **소유자 토큰이 owner 쓰기를 막는 문지기가 된다** — `X-Boring-Owner-Token` 헤더(`drudge/src/serve/owner.rs`): 엔진의 `BORING_OWNER_TOKEN` 과 일치하는 호출만 owner 를 작가·판정으로 주장할 수 있고, owner 가 쓴 노트는 owner 만 대체·수정할 수 있다(`gated_by`·`may_rewrite`). 일치하지 않으면 `owner (as author or judge) needs the owner door token in x-boring-owner-token` 으로 거절되고 거절 사유가 이벤트로 남는다 — 소유자 없이 자동 증류되는 노트를 누군가 지우거나 맘대로 고치지 못하게 하는 것.

...
### Added
- **소유자 토큰이 owner 쓰기를 막는 문지기가 된다** — 목적은 owner 가 쓴 노트가 owner 만 대체·수정하게 하는 것이고, owner 가 아닌 호출이 owner 를 작가·판정으로 주장하지 못하게 하는 것이다(`drudge/src/serve/owner.rs` — `may_rewrite`·`refused_supersedes`).
- **agents/memory — LangChain 검색기·LangGraph 저장소·Deep Agents 기억** — `BoringRetriever`(agents/memory/retriever.py)는 LangChain `BaseRetriever` 로 엔진 `/search` 한 번을 엔진 순서 그대로 돌려주고(재랭크·재임베딩 없음, 실패는 빈 목록이 아니라 raise), `BoringStore`(agents/memory/store.py)는 LangGraph `BaseStore` 로 값이 바뀐 put 한 번이 엔진 `/remember` 한 번(값이 바뀌면 현재 노트를 `supersedes` 에, delete 는 거절), `boring_deep_agent`(agents/memory/deep_agent.py)는 Deep Agents 의 `/memories/` 에 BoringStore·recall tool 에 BoringRetriever 를 꽂는다 — 에이전트가 기억에 쓴 것이 우리 노트가 되어 같은 채점·판정을 받는다.
- **MCP recall 이 대체된 노트에 새 노트 이름을 붙인다** — 회상문에서 대체된 노트 줄이 `- [wiki-0001.md (superseded by wiki-0002.md)] …` 로 나온다(세션 주입과 같은 꼴). 노트는 빼지 않고 순서도 그대로, 조회는 /search 와 같은 `store.superseded_by` 한 번이고 실패하면 -32603 으로 보인다. vector off 는 그대로.
- **데이터 이전 대조 검사기 — 사본 둘을 물린 임시 엔진으로 이전 전후를 잰다** — `scripts/parity-harness.sh {up|down}` 이 기준 DB(PARITY_BASE_DB, 기본 boring_snapshot_0923)에서 사본 둘(boring_parity_a/b)을 만들고(createdb+pg_dump|pg_restore — 기준에는 읽기만, 엔진이 기준에 붙는 일은 없다) 임시 엔진 둘(parity-a :7701, parity-b :7702 — docker run, compose 는 name 고정이라 운영을 재생성하므로 금지)을 띄운다. 엔진은 라이브 boring-drudge 와 같은 자리의 설정을 쓴다 — worktree 의 boring.json 을 /app/boring.json 에 읽기 전용으로 마운트하고 BORING_CONFIG 로 가리키고 TZ=Asia/Seoul(기본 boring.json 의 allow_company_origin=false 면 회사 출처 프로젝트 카드가 전부 비어 ② 가 눈먼다 — docker inspect boring-drudge 가 정본). 임시 볼트를 `/parity-vault` 에 마운트해 eval 픽스처를 양쪽에 대칭 수집(/vault 면 prune 이 사본을 통째로 지운다). `scripts/data-parity.py` 가 네 줄(① 현재 기록 집합 — canon 접기 후 대칭차 ② 활성 프로젝트마다 카드 다섯 — 철자 접기·동점 순서 접고 내용 비교, 레지스터 SQL 의 동점 정렬이 계획 의존적이라 사본끼리 순서가 갈림(2026-09-23 통제군 실측) ③ recall@3 — data/eval/run_eval.py ④ 판정 간선 handed·used·contested·supersedes 종류별 개수 + event_log 행 수)을 재고 모두 같으면 종료코드 0, 차이 1, 엔진·계기(psql·run_eval 의 'search failed') 불통 2 — 차이 상위 5건은 data/loop/parity-<시각>.json(gitignore). run_eval 은 /search 실패를 삼키고 Recall 줄까지 찍으므로(run_eval.py:44) 'search failed' 가 있으면 0/22 를 수치로 읽지 않고 계기 불통(2) — 임베더 다운 상태에서 /health 만 살아 있어도 0/22=0/22 로 빠지는 것을 막는다. up 은 마운트 대상 boring.json 부재 시 exit 2(docker 가 파일 대신 디렉터리를 만들어 조용히 깨지므로). 통제군(묻변경) 0·위반 변이(현재 기록 값 한 줄, 회사 출처 risk 한 줄로 ② 차이 1, 죽은 검색으로 2) 실측. 운영 DB·엔진·문·컨테이너 무접촉. 단위 시험 9개(scripts/test_data_parity.py).
- **카드에 「오늘 할 일」 칸 — 첫 실행 부류는 갈라진 주어 합치기** — 카드 그래프에 `read_repairs`(문 GET, 상위 3묶음)와 `execute_repair`(문 POST, `record_verdict` 분기) 가 붙는다. 실행 칸 버튼(실행·보류·거절)은 조언 칸과 idx 공간을 공유한다(실행 칸이 0..k-1, 조언 칸이 k..) — 실행 칸의 「실행」만 문의 새 쓰기 라우트를 부르고, 나머지는 이벤트만 남긴다. 머리줄에 `남은 묶음 n`(어제 합쳤으면 `· 어제 합친 행 m`), 실행 칸이 비면 「오늘 할 일」 머리도 없다. 낱말은 en/ko/ja `card_i18n` 세 언어. `card_view.py` 의 `.get(…, "•")` 폴백도 직접 인덱스로 정리(레지스터 4종이 이미 다 있어 폴백이 죽은 코드였다). 시험 4개(AC4~7).
- **문에 첫 쓰기 라우트 — 갈라진 주어 묶음을 읽고 합친다** — `GET /repairs/split-subjects` 가 canon 이 같아지는데 노트 표기가 갈린 claim 주어 묶음을 rows 내림차순으로 준다(`limit` 1..50, 기본 3). `POST /repairs/split-subjects {"subject"}` 는 그 묶음의 두 표기 claim 행을 지우고 해당 노트의 `sha` 를 비워 엔진 `/sync` 가 새 canon 으로 다시 읽게 한 뒤 재집계해 돌려준다 — sync 실패는 502 JSON 이되 이미 커밋된 삭제·재읽기 수는 응답에 남는다(조용한 롤백도 성공도 아님). `subject_merged` 이벤트 1건. 시험 2개(AC2~3, 스텁 커서로 DELETE→UPDATE→commit→sync→재집계 순서 단언).
- **엔진 canon 이 공백·언더스코어·하이픈을 하나로 접는다** — `foodspring front`/`foodspring-front` 처럼 표기만 다른 360묶음(29,985행)이 갈라져 있던 원인: `canon()` 이 공백만 접었다. 이제 `[\s_-]+` 를 `-` 하나로(저장소 슬러그 표기와 맞춘다). 기존 단위시험 기대값 갱신 + 신규 단언 2개.
- **카드가 카드답게 — v2 모양, 채택·보류·거절** — 제안 한 줄이 다섯 블록: divider · context(꼬리표: 그림+레지스터 이름+`wiki-NNNN`+순번/전체) · section(조언+병목, 라벨 없이) · rich_text(근거 인용, `rich_text_quote` — Slack context 블록은 rich text 를 못 받아 최상위로 뺐다. 근거 둘이면 한 블록 안에 사이 빈 줄 하나로) · actions(채택 style=primary·보류 무색·거절 style=danger) 또는(판정 뒤) context(✓ 채택/… 보류/✕ 거절). 목록 이름이 영문 키 대신 그림+우리말(재발🔁·위험⚠️·정체🧊·다음➡️, en/ja 도 `card_i18n.REGISTER_LABELS`)로 나오고, 프로젝트 구분은 header 대신 section(`　\n*label*`)이라 카드 상단 타이틀만 유일한 header 로 남는다. 버튼 value·인용·꼬리표 어디에도 `/vault/wiki/...` 전체 경로가 없다 — `wiki-0536` 형태만. 한 메시지 50블록 상한을 넘기면 조용히 잘라내지 않고 마지막 context 줄에 넘친 제안 수를 적는다. 시험 6개(AC3~8, 변이 6종 전부 사망 확인) + 기존 82개 갱신.
- **카드가 프로젝트별로 갈리고 세 언어로 나오며 제안을 문제지로 남긴다** — 문의 새 `GET /projects?active_days=`(DB 직접, 엔진의 무필터 `/projects` 와 별개 — `active_days` 없이 부르면 그대로 프록시)가 최근 14일 활성 프로젝트를 문서 수 내림차순으로 답하고, 카드는 그 목록 + 무소속("") 마다 레지스터 넷을 따로 불러 `Proposal.project` 를 채운다. `card_core.merge_project_candidates` 가 프로젝트를 넘나드는 주어 중복만 걷어내 모델 호출 예산(최대 8회)이 활성 프로젝트 수에 안 늘게 하고, `build_blocks` 는 제안을 프로젝트 제목 아래 묶는다(무소속은 통 이름). 표시 문자열(제목·버튼·판정 표시·머리줄·통 이름) 전부가 새 `agents/slack/card_i18n.py` 의 en/ko/ja 표 하나에서 나오고, 언어는 `card_core.resolve_lang(boring_config.note_lang())` — `note_lang` 이 ko/ja/en 이 아니면(예: auto) English 로 접힌다. 조언 프롬프트에는 증류기와 같은 방식으로 언어 지시 한 줄이 붙는다. 제안마다 `card_proposal` 이벤트, 버튼마다(미뤄 포함) `card_verdict` 이벤트가 남아 다음 채점(C14)의 문제지가 되고, `card_core.suppressed()` 가 지난 7일 안에 해/빼 판정을 받은 (note, evidence.note, evidence.line) 짝을 다시 올리지 않는다(미뤄는 흔적이 없어 억제 대상이 아니다) — 판정 이력을 못 읽으면(이벤트 창이 잘렸거나 door 가 죽었으면) 카드 자체가 거부된다. `CARD_WAIT_HOURS`(기본 23)가 지나면 안 누른 제안을 그대로 두고 조용히 끝난다. `scripts/schedule-card.sh` 가 08:00 launchd 등록 전례(`schedule-maintenance.sh`)를 따른다. 시험 71개(card_core/card) + 37개(door), 네트워크 없음.
- **브리핑이 비서가 된다 — 과거 결정을 찾아 병목을 짚고 좌표 달린 조언을 낸다** — 카드 그래프의 `propose` 노드가 `advise` 로 바뀐다: 오늘 레지스터(재발·위험·정체 — `next_actions` 는 이미 실행 가능해 제외)의 후보 주어를 지난 승인 「아직」 우선 순서로 하나씩 골라, `/search`(claims 포함, 새 `search` 협력자 — 라이브 `DrudgeClient.search`) 로 과거 기록을 끌어오고, 로컬 gemma4:12b(`format=json`, 구조화 출력 없이 원문 텍스트) 에 후보 하나당 한 번 물어 `Advised = Proposal | NotWorth` 판별 유니온 답을 받는다. `card_core.parse_advised` 가 유일한 신뢰 경계: evidence 의 `quote` 가 그 노트(새 `read_note` 협력자, 호스트 `BORING_VAULT_DIR` 에서 읽음) 본문에 공백만 다르게라도 그대로 있으면 `line` 을 계산해 채우고, 없으면 그 evidence 를 버리며, evidence 가 0 이 되면 후보 전체를 `Ungrounded` 로 버린다 — 모델이 지어낸 좌표는 절대 믿지 않는다(실험 1·wiki-1064). 셋이 차거나 8회를 채우면 멈추고, `NotWorth`·`Ungrounded` 는 값으로 건너뛸 뿐 예외를 던지지 않는다. `resolve` 노드는 그대로 주어→note 를 door 로 풀되, 재-propose 없이 fewer-than-three(0 포함)로도 카드를 낸다 — 실험 1 의 회수 정밀도(1/5)를 반영해 "빈 카드도 판단"이라는 wiki-1765 정의를 따른다. 카드 블록에 제안마다 병목 한 줄과 근거 줄(`wiki-NNNN L<n> 「quote 앞 60자」`, 최대 2개)이 붙는다. `CARD_DRY_RUN=1` 이면 post_card 직후 멈추고 제안 JSON 을 stdout 에 내며 전송·handover·이벤트를 하지 않는다(기본값 없음 — unset 이면 실제 전송). 라이브 실측(2026-09-22, 문 :7710 + gemma4:12b): 첫 시도는 프롬프트가 노트 경로를 `[…]` 로 보여줘 모델이 대괄호까지 인용에 옮겨 8/8 호출 전부 근거 거부(경계 조건과 정확히 일치) — 프롬프트에서 대괄호를 없애자 6회 호출·통과 3·거부 이유 전부 "구체성 부족"(not_worth)으로 정상화됐다. 시험 40개, 네트워크 없음.
- **판정은 노트에, 확인은 엔진 계기가** — 레지스터의 `sources` 는 claim 주어(예 「3차 창 샘플링」)라 카드의 판정 간선이 `doc:<주어>` 에 붙던 사이클 6 결함을 닫는다. 문에 읽기 둘째 `GET /claim-source?subject=…` (스냅샷 `door_routes` 등록, contract-parity 는 여전히 `http_routes` 만 봄): 주어의 현재 claim(`superseded_at is null` 중 `valid_from` 최신 1건) 의 `source_path` 를 풀어 `{subject, note, valid_from, candidates}` 로 답한다 — 주어 하나가 경로 여럿에 걸린 현재 claim 이 1,470건(2026-09-22 실측)이라 후보는 응답에 함께 실린다. 선택은 순수 함수 `claim_source.pick_current` (`agents/door/claim_source.py`), SQL 은 파라미터 바인딩. 카드(`agents/slack/card.py`)의 새 resolve 노드가 propose 뒤 주어를 경로로 푼다: Proposal 은 `subject`+`note` 를 갖고(ADT 교정), 못 푸는 주어는 「근거 노트 없음」 도 카드에 안 실린 채 값으로 떨어지고, 셋 미만이면 남은 후보로 re-propose 1회, 그래도 0이면 거부(exit 3) — 조용한 빈 카드는 없다. recurrences 의 경로 성분은 문 없이 자기 자신으로 풀린다. handover·consumption·슬랙 버튼 value 는 전부 `/vault/wiki/…` 경로다. 더불어 카드 머리에 지난 48시간 「해」 대조 줄 — `지난 승인 N · 했다 a · 아직 b`: 지날 approved 의 note 경로가 오늘 레지스터 주어들을 resolve 한 집합에 아직 나오면 「아직」(다시 제안 후보), 사라졌으면 「했다」. 대조는 엔진 event_log 의 `card_confirmation` 이벤트 1건(done·pending·지난 카드 세션)으로 남는다 — 확인은 실행한 쪽의 말이 아니라 엔진 계기의 기록이다. `BORING_DOOR_URL` 이 카드에도 필수가 되어 없으면 기동 거부(exit 2), 문이 죽어도 exit 3 + stderr 한 줄. 시험: test_door 30개·test_card_core 26개, 네트워크 없음.
- **승인한 것이 다음 세션 시작 카드에 놓인다 — 문의 첫 프록시 아닌 일** — `GET /approved?since_hours=24` (문 :7710, 엔진 프록시가 아니라 문 자체의 첫 라우트 — Python 이 저장소를 처음 읽는 자리). 엔진 그래프 `edge` 표에서 `session:slack:<channel>:<ts>` 세션이 `used`/`contested` 로 판정한 노트를 돌려준다: 표에 시각 컬럼이 없어(2026-09-22 실측) 세션 이름의 마지막 `:` 뒤 Slack epoch ts 가 시각이고, 그걸로 창을 자른다(`agents/door/approved.py` — 파싱·필터·정렬은 순수 함수, ts 없는 이름은 예외가 아니라 `Skipped` 값). `DOOR_PG_DSN` 이 없으면 503 `store not configured` — 빈 목록 200 으로 조용히 넘어가지 않는다. 계약 스냅샷에 `door_routes` 키가 새로 생겨 문 라우트를 등록하고, contract-parity 는 여전히 `http_routes`(엔진 것) 만 본다. Claude Code SessionStart 훅은 `BORING_DOOR_URL` 이 있고 문이 살아 있으면 카드 위에 「오늘 승인한 것 (N)」 절을 붙이고, 문이 없으면 stderr 한 줄로 말하고 카드는 그대로 낸다. 문 컨테이너엔 `psycopg[binary]` 가 들어가고(compose 가 서비스 이름 DSN 을 넣어준다) 훅은 stdlib 그대로다.
- **아침 카드 — 제안 셋을 보내고 버튼이 곧 판정이 된다** — `make card` 가 LangGraph 그래프 하나(g_card)로 엔진 레지스터 넷(next_actions·risks·stalled·recurrences)을 읽고, 로컬 gemma4:12b 가 제안 셋을 골라 Slack Block Kit 카드를 `SLACK_CARD_CHANNEL` 에 보낸 뒤 `interrupt()` 로 멈춰 사람 결재를 기다린다. 버튼(해·미뤄·빼) 이 눌리면 `Command(resume=…)` 로 같은 그래프가 이어 돌아 판정을 내리고 — 해=used, 빼=contested, 미뤄=기록만 — 보낸 것과 판정이 한 상태에 남는다. 제안은 레지스터에 있는 것만: `parse_proposals` 가 `source_note` 가 그 레지스터의 sources 에 없으면 카드 없이 거부하고, 버튼 payload 의 이상은 `Rejected` 값으로 돌려 받는다(`agents/slack/card_core.py` + 시험 17개, 네트워크 없음). 실측: 레지스터 수집→구조화 출력까지 16초.
...
### Changed
- **카드가 후보 자신의 노트를 못 찾았으면 읽어 와 근거 맨 앞에 싣는다** — 재발 명부의 주어는 노트 경로라 /search(그 경로)가 후보 본인의 노트를 돌려주지 않아 본문이 프롬프트에 못 실리고 모델이 「제공된 노트가 비어 있거나 없다」로 거절하던 (09-26 드라이런 wiki-0576·wiki-0697) 것을, 프롬프트를 만들기 전 `card_advice.check_sufficiency` 가 히트 안에 자기 노트가 있는지 확인해 빠졌으면 `read_note` 로 직접 읽어 첫 히트로 싣는다(스니펫은 프론트매터 뒤 본문 첫 ~600자, 인용 검증은 본문 전체). 재발 후보는 경로 검색 자체를 건너뛰고 노트 제목 검색으로 관련 히트를 최대 2개 더 얹는다. 주어가 안 풀리면 추측하지 않고 사유만 적고, 노트를 못 읽어도 지금처럼 진행한다. `AdviseStats.refetched_own_note`·`sufficiency_reasons` 이 dry-run JSON 에 남고 후보마다 추가로 읽는 것은 read_note 한 번, 모델 호출은 그대로 한 번. 시험 14개, 변이 넷(검사 고정·자기 노트를 뒤에·본문 누락·재발 경로 검색) 모두 시험이 잡음.
- **쉬는 노트는 모델을 부르기 전에 건너뛴다** — 판정 이력(7일 창)을 advise 앞 `read_history` 노드에서 한 번 읽어 상태에 싣고, 조언 루프가 후보를 잡기 전에 그 노트가 쉬는 중이면(사흘 안에 버튼 없이 지난 제안, 7일 안에 해/빼 판정 — `card_verdicts.resting_notes`) 검색·모델 호출 없이 건너뛴다. 억제가 모델 호출 뒤에만 걸려 호출 8번이 이미 본 후보에 쓰이고 조언 칸이 빌 수 있었다. 미뤄 판정 노트는 쉬지 않아 그대로 호출되고, 정밀한 (note, 근거 노트, 근거 줄) 억제는 `resolve` 의 `suppressed` 가 그대로 맡는다. `AdviseStats.skipped_resting` 이 새 수치를 남기고 dry-run JSON 에도 실린다. 시험 3개(순수, 스텁) + 변이 2종 사망 확인.
- **버튼 없이 지나간 제안은 사흘 쉬었다 다시 올라온다** — 카드에 올려졌는데 아무 판정도 받지 못한 (note, 근거 노트, 근거 줄) 짝은 72시간(`REST_HOURS`) 동안 다시 제안되지 않는다: wiki-1486 위험이 08:23 카드에 실렸는데 버튼이 없어 매일 아침 그대로 돌아오던 것(2026-09-26 드라이런)이 계기. `card_live._live_past_verdicts` 가 card_proposal↔card_verdict 묶음을 판정받은 짝과 무응답 짝(`PastUnansweredPair`, 시각은 card_proposal 본인의 observed_at) 두 목록으로 함께 돌려주고 `card_verdicts.suppressed` 가 무응답 짝을 쉼 규칙으로 뺀다 — 판정이 하나라도 있는 짝(미뤄 포함)은 쉼 대상이 아니어서 미뤄 제안은 그대로 다시 온다. 시험 4개.
- **레지스터 넷(결정·위험·다음 할 일·정체)이 `limit` 을 받는다** — 지역 12B 에이전트가 4만 자 레지스터 출력에서 잘려 주간 브리핑이 실패한(2026-09-26) 것에 대한 것: MCP·HTTP 두 문이 `limit`(1..50, 기본 50) 을 받고 범위 밖(0·음수·초과·비정수)은 거절한다. 기본이 그대로라 문을 경유하는 기존 소비자(카드)는 같은 결과를 본다.
- **forget 이 닫히고 동기화의 정리도 멈춘다** — 이전 기간 동안 `forget` 는 모든 호출을 거부하고 아무것도 지우지 않는다(거절은 `forget_refused` 이벤트로 남는다, `drudge/src/serve/mcp.rs`). 정정은 `remember` + `supersedes` 로. 동기화도 사라진 노트를 prune 하지 않고(`prune_skipped` 이벤트, `drudge/src/ingest.rs`), `scripts/dedup-wiki.py --apply` 도 같은 이유로 거절된다 — 자동 증류 시대에 기계가 지우는 일을 전부 멈춘 것(#398).
- **카드 코드가 관심사별 여섯 모듈로 갈린다** — `agents/slack/card_core.py`(740줄)와 `card.py`(793줄)를 `card_types`(값)·`card_registers`(레지스터)·`card_advice`(조언 프롬프트·근거 검증)·`card_verdicts`(억제·이벤트 필드·버튼 파싱)·`card_view`(Block Kit)·`card_live`(door/엔진 협력자)로 가른다 — `card_core` 라는 이름은 남기지 않는다. `card_types` 는 다른 `card_*` 를 import 하지 않고, `card_live` 는 `card_types` 만 의존해 `card.py → card_live` 단방향을 지킨다. 동작은 그대로다(고정 픽스처로 build_blocks 출력 바이트 동일 확인).
...
### Fixed
- **대체로 내린 기록이 다시 열리지 않는다** — `Store::open` 마다 도는 2026-09-17 unseal 백필과 `upsert_claim` 의 「latest unseal」이 `supersedes` 간선이 봉인한 행까지 열어 끝난 항목이 `next_actions` 에 돌아왔다. 세 자리(백필·fact unseal·fact 가 아닌 unseal) 모두 `EXISTS(edge.kind='supersedes' AND dst = 'doc:' || source_path)` 가드로 간선의 대상 노트를 건드리지 않는다 — 옛 노트 자기 행의 재upsert 와, 다른 노트가 같은 fact 슬롯에 더 오래된 valid_from 을 쓰는 것, 두 재열림도 같은 가드로 막는다. fact 슬롯의 최신값은 이제 살아 있는 노트 행만으로 잰다 — 대체된 노트가 최신 `valid_from` 자리를 계속 차지해 다른 노트의 더 오래된 행이 봉인만 되고 칸의 현재 행이 0 이 되던 구멍이고, 내리기(`record_supersedes`·`compact`)는 그 노트가 건드린 fact 슬롯마다 남은 살아 있는 행 중 최신을 다시 현재로 올린다(남은 살아 있는 행이 없으면 칸은 빈다). 시험 5개.
- **`remember` 가 대체 경로 표기를 알아듣는다** — `supersedes` 가 `/vault/wiki/x.md` 한 표기만 받아 `wiki/x.md`·`x.md` 는 document 행을 못 찾아 아무것도 연결하지 않았고, 답이 그 사실을 말하지 않아 대체가 끝난 줄 알았다. 경계 파싱이 세 표기를 `/vault/wiki/x.md` 로 접고(나머지는 그대로 둬 unknown 으로 잰다), 못 연 경로는 MCP `remember` 답 뒤에 ` · supersedes linked N, not found M (경로…)` 가 붙는다 — supersedes 를 안 준 답은 기존과 바이트 동일. 시험 3개.
- **대체한 노트가 옛 노트의 현재 기록을 내린다** — `supersedes` 간선이 생길 때 옛 노트의 현재 claim 전부가 새 노트의 최신 claim 시각으로 봉인되며, kind 와 무관하게 끝난 항목이 `next_actions` 에 다시 올라오지 않는다(owner 가 쓴 옛 노트는 owner 가 쓴 새 노트만 내린다 — 문지기 거절과 같은 규칙의 저장소 쪽). 봉인 단계보다 먼저 쓰인 간선은 `compact` 가 매번 걸어 메우고 봉인 수를 `CompactSummary` 에 적는다. 시험 4개.
- **doctor 의 배포·호스트 CLI 어긋남이 빌드가 읽는 파일만 본다** — #404(drudge/CLAUDE.md, 문서 한 커밋)가 실제 배포본(ffd86bf6)과 상관없이 ✗ DEPLOY DRIFT·✗ HOST CLI DRIFT 둘 다 울렸다 — 어긋남 비교 대상을 drudge/ 전체에서 빌드 입력(Dockerfile·rust-toolchain.toml·Cargo.toml·Cargo.lock·build.rs·src/)으로 좁혔다. 시험 3개(문서만 고친 커밋은 엔진·호스트 CLI 둘 다 통과, src/·Dockerfile 을 고친 커밋은 실패).
- **doctor 가 아침 카드가 오늘 나갔는지 말한다** — 09-24~26 세 아침이 연속 실패했는데 doctor 에 카드 항목이 하나도 없어 발견이 늦었다. `scripts/schedule-card.sh` 의 시작·종료 마커에 ISO 시각을 붙이고, card.py 가 게시 직후 `[card] posted ts=` 한 줄을 남긴다(finished 는 CARD_WAIT_HOURS 23시간 대기 뒤에야 찍혀, 성공한 실행이 대부분의 날 동안 finished 없이 남는다). doctor 는 `${BORING_CARD_LOG}` 의 마지막 시작 시각과 뒤따르는 finished·posted 줄로 성공·실패·미실행을 가리고, 마지막 시작이 지난 08:05(08:00 실행 + 발송 여유)보다 오래됐으면 「안 나감」으로 본다. 게시 뒤 비정상 종료는 ✓ 에 경고 한 줄, 날짜를 못 읽으면 「모름」. 시험 10개 + 변이 2개.
- **아침 카드가 실패하면 같은 DM 에 이유 한 줄을 보낸다** — 09-24~26 세 아침이 거부 둘·예외 하나로 연속 실패했는데 전부 /tmp 로그에만 남아 소유자는 알 길이 없었다. 문 불통(exit 2)과 card.py 비정상 종료가 모두 지나는 유일한 자리인 `run_card` 에 알림을 둔다: 문이면 문 URL, 카드면 이번 실행 stderr 마지막 한 줄을 `오늘 아침 카드를 못 보냈다 — exit N · 사유` 로 보낸다. 알림 자체가 실패하면 stderr 로 말한다 — 슬랙은 실패에도 HTTP 200 이라 `"ok":true` 만이 성공이다. 카드 본문도 이제 `text=` 폴백을 받아 폰 푸시에 읽힌다. 시험 4개 + 변이 1개.
...
- **compose 가 두 컨테이너에 `BORING_OWNER_TOKEN` 을 넘긴다** — 엔진·문·카드는 `BORING_OWNER_TOKEN` 을 환경에서 읽는데 compose 가 `boring-drudge`·`boring-door` 에 이 값을 넘기지 않아, `.env` 에 토큰이 있어도 컨테이너 안에서는 카드 판정 버튼·정정 같은 소유자 쓰기가 전부 거절되던 것(#399).
- **근거 노트도 건넨 것으로 기록하고, 미리보기·인용 검사에 시험을 붙인다** — 독립 검증에서 REVISE 로 돌아온 사이클 11의 다섯 결함. (1) `handover` 가 제안의 `note` 만 건네 카드가 실제로 인용한 evidence 노트를 엔진이 몰랐다 — `card_core.handover_paths` 가 제안 note 와 evidence note 전부의 중복 없는 순서 보존 합집합을 계산하고, subject 가 resolve 된 note 와 evidence 가 다른 경우를 새 시험이 고정한다. (2) `CARD_DRY_RUN=1` 배선을 실제로 실행하는 시험이 없어 `_run_dry` 가 라이브 send·handover 로 잘못 배선돼도 40개가 전부 통과하던 결함 — `DryRunWiringTests` 가 협력자 전부를 스텁하고 라이브 함수 호출 0회·dry 함수 호출을 단언한다(스크래치 사본에 변이를 넣어 실패를 본 뒤 원복, 확인함). (3) `_find_quote_line` 이 첫 12자만 맞아도 그루딩되는 변이가 마찬가지로 40개를 다 통과 — 실제 노트 줄과 앞부분만 같고 나머지는 지어낸 인용을 `Ungrounded` 로 고정하는 시험을 추가(같은 방식으로 변이·복원 확인). (4) `advise` 가 계산한 `NotWorth.reason`·`Ungrounded` 사유가 그 자리에서 버려져 실행 결과를 재현할 수 없었다 — `card_core.AdviseStats`(calls·proposals_passed·not_worth·ungrounded·사유 목록)를 그래프 상태에 실어 dry-run JSON 에 그대로 낸다. (5) 검색 결과는 있었는데 노트 본문을 하나도 못 읽은 경우(예: 잘못된 `BORING_VAULT_DIR`)가 조용한 「제안 0」과 구분되지 않았다 — advise 가 그 상황을 처음 만나면 `BORING_VAULT_DIR` 값을 실어 stderr 한 줄을 낸다(폴백 없음, 말만 한다). 시험 45개.
- **카드가 문 장애를 「했다」로 읽지 않고 「아직」을 먼저 제안한다** — 사이클 8 결함: 지난 승인 대조에서 /claim-source 의 5xx·불통이 오늘 주어 resolve 의 404 「없음」과 같은 값으로 뭉개져, 확인하지 못한 지난 승인이 「했다」로 잡혔다. 이제 대조는 propose 앞 cross_check 노드에서 끝나고 404 만 부재의 근거가 되며, 실패는 카드 머리 「확인불가 k」와 event_log 의 unknown=k 로 남는다. /approved 자체의 불통은 값이 아니라 실행 실패 — 카드 없이 exit 3. 「아직」인 주어는 프롬프트의 「우선 후보」 절로 먼저 제안되고, 확인 이벤트는 post_card 의 send 성공 뒤에 1건만 남긴다. 시험: test_card_core 35개, 그래프 단언은 draw_ascii 대신 get_graph().nodes 의 키 집합 — CI 의 grandalf 임포트 결함(run 35622206939) 폐기.
- **문의 /approved 가 since_hours 경계를 지킨다** — 사이클 7 결함: `/approved?since_hours=99999999999999` 가 `timedelta(hours=…)` OverflowError 로 500 텍스트를 냈다. 이제 하한 1·상한 8760(24×365), 경계 밖(0·-1·1.5·99999999999999·8761)은 400 JSON — 판정은 `approved.check_since_hours` 순수 함수. `since_hours=0` 도 400: 빈 창의 빈 목록은 "승인된 것 없음"처럼 읽혀 저장소가 말한 사실이 아니기 때문.

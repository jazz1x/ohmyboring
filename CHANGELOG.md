...
### Fixed
- **대체한 노트가 옛 노트의 현재 기록을 내린다** — `supersedes` 간선이 생길 때 옛 노트의 현재 claim 전부가 새 노트의 최신 claim 시각으로 봉인되며, kind 와 무관하게 끝난 항목이 `next_actions` 에 다시 올라오지 않는다(owner 가 쓴 옛 노트는 owner 가 쓴 새 노트만 내린다 — 문지기 거절과 같은 규칙의 저장소 쪽). live 에 이미 있는 간선(wiki-1945→wiki-1759 등, 봉인 없는 현재 claim 4건)은 `compact` 가 매번 간선을 걸어 매꾸고 봉인 수를 `CompactSummary` 에 적는다. 시험 4개.

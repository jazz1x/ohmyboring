...
### Fixed
- **doctor 의 배포·호스트 CLI 어긋남이 빌드가 읽는 파일만 본다** — #404(drudge/CLAUDE.md, 문서 한 커밋)가 실제 배포본(ffd86bf6)과 상관없이 ✗ DEPLOY DRIFT·✗ HOST CLI DRIFT 둘 다 울렸다 — 어긋남 비교 대상을 drudge/ 전체에서 빌드 입력(Cargo.toml·Cargo.lock·build.rs·src/, drudge/Dockerfile COPY 19·29·30 과 `cargo build --release` 가 읽는 그 목록)으로 좁혔다. 시험 2개(문서만 고친 커밋은 통과, src/ 고친 커밋은 실패) + 변이 1개(경로를 drudge/ 로 되돌리면 문서 케이스가 죽는 것 확인).

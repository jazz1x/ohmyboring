...
### Added
- **데이터 이전 대조 검사기 — 사본 둘을 물린 임시 엔진으로 이전 전후를 잰다** — `scripts/parity-harness.sh {up|down}` 이 사본 B(boring_rehearsal_new)를 A(boring_rehearsal)에서 복제(createdb+pg_dump|pg_restore)하고 임시 엔진 둘(parity-a :7701, parity-b :7702 — docker run, compose 는 name 고정이라 운영을 재생성하므로 금지)을 띄우되 임시 볼트를 `/parity-vault` 에 마운트해 eval 픽스처를 양쪽에 대칭 수집(/vault 면 prune 이 사본을 통째로 지운다). `scripts/data-parity.py` 가 네 줄(① 현재 기록 집합 — canon 접기 후 대칭차 ② 활성 프로젝트마다 카드 다섯 — 철자 접기·동점 순서 접고 내용 비교, 레지스터 SQL 의 동점 정렬이 계획 의존적이라 사본끼리 순서가 갈림(2026-09-23 통제군 실측) ③ recall@3 — data/eval/run_eval.py ④ 판정 간선 handed·used·contested·supersedes 종류별 개수 + event_log 행 수)을 재고 모두 같으면 종료코드 0, 차이 1, 엔진 불통 2 — 차이 상위 5건은 data/loop/parity-<시각>.json(gitignore). 통제군(묻변경) 0·위반 변이(현재 기록 값 한 줄) ① 차이 1 로 실측. 운영 DB·엔진·문·컨테이너 무접촉. 단위 시험 7개(scripts/test_data_parity.py).
